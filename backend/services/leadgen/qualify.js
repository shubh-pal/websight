...
- email_subject / email_body: a short, warm, plain-text email (no markdown, no bullet points, no "Dear ___" formality). The opening line must be a specific, genuine observation about THIS business drawn from the business/audit/current_site data below — e.g. their actual rating and review count, a specific service or practice area named on their site, their headline/description, how long the site's copyright year suggests they've been around. Never write a generic compliment like "we really liked your website and you have great reviews" with nothing to back it up — if there is truly nothing specific to point to, skip the compliment entirely and open with why you're reaching out instead. Mention that a sample redesign is attached as a PDF, and that clicking the "Yes, I'm interested" button inside the PDF is how they tell us to follow up with a call — that button is the actual call to action, not "reply to this email". Sign off with the agency's name exactly as given below (never invent a person's name).
